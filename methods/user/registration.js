    ...
    uni.request({
        url: baseUrl + "/user/registration", method: "POST", data: newUser, header: {
            ...
            if (res.statusCode == 200) {
                ...
                if (res.data === "已注册") {
                    uni.showToast({
                        title: '该号码已注册！', icon: "none", duration: 1000, mask: true,
                    })
                } else {
            ...
            console.log(err)
            uni.showToast({
                title: '发送网络请求失败！', icon: 'none', duration: 1000, mask: true,
            })
        }
    })
}
...
export default function registration(newUser, confirmPwd) {
    if (newUser.phone == '' || newUser.nickname == '' || newUser.password == '' || confirmPwd == '') {
        if (newUser.phone == '') {
            uni.showToast({
